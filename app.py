...
SOURCE_BAG_LOCATIONS = [
    "source",
    "private/external-preservation",
...
@cache
def _is_file_too_large(
    file_sizes: int or tuple[int, ...],
    max_size: int = LAMBDA_MAX_MEMORY_FOR_DERIV,
...
def _generate_pdf(
    ...
    if _is_file_too_large(
        image_sizes, max_size=LAMBDA_MAX_MEMORY_FOR_PDF, buffer_ratio=0.3
    ):
        logger.error(
            f"Total size of derivatives is more than half of available memory: {sum(image_sizes)}"
        )
        ...
        pdf.save(
            ...
            append_images=(
                ...
                for image_path in image_paths[
                    1:
                ]  # skipping first image used to bootstrap pdf
            ),
        )
...
def images_source(bag: str, location_and_bag: str = "") -> list[dict]:
    ...
    location_and_bag = (
        _find_source_bag(bag)["location"] if not location_and_bag else location_and_bag
    )
...
def images_derivative(bag: str, scale: float = DEFAULT_IMAGE_SCALE) -> list[dict]:
    ...
    return list(
        _images(
            f"derivative/{bag}/{scale}/", extensions=DEFAULT_IMAGE_EXTENSIONS + ("pdf",)
        )
    )
    ...
    try:
        return list(
            set(
                [
                    item["Key"].split("/")[2]
                    for item in s3_client.list_objects(
                        Bucket=S3_BUCKET, Prefix=f"derivative/{bag}/"
                    )["Contents"]
                ]
            )
        )
...
def resize_individual(
    bag: str, scale: float, image_path: str, location: str = ""
) -> dict:
    ...
    if _is_file_too_large(
        _object_size(S3_BUCKET, f"{bag_location}/data/{image_path}"),
        max_size=LAMBDA_MAX_MEMORY_FOR_DERIV,
    ):
        raise BadRequestError("The source image is too large")

    try:
        source_image = Image.open(
            _s3_byte_stream(S3_BUCKET, f"{bag_location}/data/{image_path}")
        )
